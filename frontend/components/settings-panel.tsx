...
export function SettingsPanel() {
  ...
  const [restUrl, setRestUrl] = useState(state.settings.restBaseUrl)
  const [wsUrl, setWsUrl] = useState(state.settings.wsUrl)
  const [saved, setSaved] = useState(false)
  const [errors, setErrors] = useState<{ rest?: string; ws?: string }>({})
  ...
  const validate = () => {
    const newErrors: { rest?: string; ws?: string } = {}
    if (restUrl && !restUrl.startsWith("http")) {
      newErrors.rest = "Must start with http:// or https://"
    }
    if (wsUrl && !wsUrl.startsWith("ws")) {
      newErrors.ws = "Must start with ws:// or wss://"
    }
    setErrors(newErrors)
  ...
  const handleSave = () => {
    ...
    const settings = { restBaseUrl: restUrl, wsUrl: wsUrl }
    ...
    <div className="flex flex-1 overflow-hidden">
      <main className="mx-auto w-full max-w-2xl overflow-y-auto p-8">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Configure backend connection and video upload for the parking system.
        </p>
